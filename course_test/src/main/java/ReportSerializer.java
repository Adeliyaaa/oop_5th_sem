...
public class ReportSerializer implements JsonSerializer<Report>, JsonDeserializer<Report> {
    ...
    public JsonElement serialize(Report o, Type type, JsonSerializationContext jsonSerializationContext) {
        ...
        jsonObject.add(STUDENTS, jsonSerializationContext.serialize(o.getStudents().toString()));
    ...
    public Report deserialize(JsonElement json, Type type, JsonDeserializationContext context) {
        ...
        Report report = new Report(context.deserialize(jsonObject.get(GROUPNUMBER), int.class),
                context.deserialize(jsonObject.get(SUBJECT), String.class));
        report.setProfessor(context.deserialize(jsonObject.get(NAME), String.class),
                context.deserialize(jsonObject.get(SURNAME), String.class),
                context.deserialize(jsonObject.get(LASTNAME), String.class));
        report.setStudents(context.deserialize(jsonObject.get(STUDENTS),
